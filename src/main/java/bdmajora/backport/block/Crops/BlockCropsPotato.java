...
public class BlockCropsPotato extends BlockFlower implements IBonemealable {
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
		if (meta < MAX_GROWTH_STAGE) {
			return new ItemStack[]{new ItemStack(ModItems.foodPotatoRaw, 1)};
		} else {
			int potatoCount = world.rand.nextInt(3) + 1; // Random number between 1 and 3
			return new ItemStack[]{new ItemStack(ModItems.foodPotatoRaw, potatoCount), new ItemStack(ModItems.foodPotatoRaw, 1)};
		}
	}
