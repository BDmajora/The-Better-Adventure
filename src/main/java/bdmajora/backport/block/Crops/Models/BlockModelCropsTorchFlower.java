...
public class BlockModelCropsTorchFlower<T extends Block> extends BlockModelStandard<T> {
	public final IconCoordinate[] growthStageTextures = new IconCoordinate[]{
		TextureRegistry.getTexture(MOD_ID + ":block/torchflower_crop_stage0"),
		TextureRegistry.getTexture(MOD_ID + ":block/torchflower_crop_stage1"),
		TextureRegistry.getTexture(MOD_ID + ":block/torchflower_crop_stage2")
	};

	public BlockModelCropsTorchFlower(Block block) {
		super(block);
	}
	...
	public IconCoordinate getBlockTextureFromSideAndMetadata(Side side, int data) {
		return this.growthStageTextures[MathHelper.clamp(data, 0, 2)];
	}
}
