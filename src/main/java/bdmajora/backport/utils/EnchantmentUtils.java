...
public class EnchantmentUtils
{
	...
	{
		List<EnchantmentData> enchantments = new ArrayList<>();
		double costPercentage = (double) cost / backport.config.getInt("max_enchantment_cost");

		int itemEnchantability = backport.config.getInt("default_item_enchantability");
	...
	{
		double percentage = (bookshelfs + backport.START_COST_OFFSET) / (15.0 + backport.START_COST_OFFSET);
		percentage *= (enchantOption + 1) / 3.0;

		return (int) (backport.config.getInt("max_enchantment_cost") * percentage);
	}
