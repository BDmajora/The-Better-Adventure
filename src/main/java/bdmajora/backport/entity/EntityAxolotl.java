...
public class EntityAxolotl extends EntityWaterAnimal {
	...
	public EntityAxolotl(World world) {
		super(world);
		this.textureIdentifier = new NamespaceID("backport", "axolotl");
	...
	public void init() {
		...
		if (!this.isAxolotlTamed() && this.random.nextInt(100) == 0) {
			this.setAxolotlHeldItem(new ItemStack(ModItems.recordOtherSide, 1));
		}
	}
	...
	public String getLivingSound() {
		if (this.random.nextInt(3) == 0) {
			if (this.isAxolotlTamed() && this.getHealth() < this.getMaxHealth() / 2) {
				return "backport.axolotlhurt";
			}
			return "backport.axolotlidle";
		}
		return "backport.axolotlidle";
	}
